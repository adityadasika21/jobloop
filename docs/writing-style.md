# Writing style — cover letters and messages

Applies to `cover.yaml`, every `jt message` pitch, and `fit.md`. Resume bullets follow
`profile/master.yaml`'s own voice (which does use em-dashes); letters do not. `jt cover`
enforces the mechanical rules (no em-dashes, word budget, sourced company claims).
