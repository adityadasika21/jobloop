...
**FAIL** — 0 critical, 1 high · 4123 chars extracted

Keyword coverage **on extracted text**: 21% exact · 25% allowing word-form variants

| | Check | Result |
|---|---|---|
| ✅ | `text-extractable` | 4123 chars recovered by the text extractor |
...
| ❌ | `ats-keyword-coverage` _(HIGH)_ | 21% of JD requirement terms present in the EXTRACTED text |
...
`architect`, `engineers`, `enterprises`, `features`, `platforms`

## JD terms genuinely absent

Add only what is true; the rest are gaps for the ledger.

`18-30l`, `agents`, `ai-powered`, `alumni`, `ambitious`, `analytical`, `annual`, `automation`, `available`, `based`, `believe`, `big`, `bonus`, `bonuses`, `budget`, `builders`, `clarity`, `clients`, `coders`, `combining`, `comfort`, `compensation`, `core`, `create`, `cutting-edge`, `demo`, `deployed`, `designers`, `development`, `diverse`, `dynamic`, `elite`, `enterprise-grade`, `equity`, `esops`, `ex-microsoft`, `excited`, `fast-paced`, `fit`, `fixed`
