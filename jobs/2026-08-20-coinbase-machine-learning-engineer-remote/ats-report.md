...
**FAIL** — 0 critical, 1 high · 4636 chars extracted

Keyword coverage **on extracted text**: 31% exact · 34% allowing word-form variants

| | Check | Result |
|---|---|---|
| ✅ | `text-extractable` | 4636 chars recovered by the text extractor |
...
| ❌ | `ats-keyword-coverage` _(HIGH)_ | 31% of JD requirement terms present in the EXTRACTED text |
...
`domain`, `engineers`, `frameworks`

## JD terms genuinely absent

Add only what is true; the rest are gaps for the ledger.

`adk`, `advanced`, `ai-enabled`, `applying`, `architectures`, `balancing`, `bedrock`, `bridge`, `business-ready`, `capabilities`, `clearly`, `coding`, `coinbase`, `collaborating`, `communicate`, `compliance`, `computer`, `context`, `contribute`, `cross-functional`, `customer-facing`, `development`, `distributed`, `efficiency`, `ensure`, `environment`, `experiences`, `features`, `google`, `high-performing`, `human`, `improve`, `improvements`, `langsmith`, `manages`, `measurable`, `meet`, `modern`, `nlp`, `one`
